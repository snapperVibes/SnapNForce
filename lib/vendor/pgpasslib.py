...
"""
# https://pypi.org/project/pgpasslib/
...
def getpass(host=DEFAULT_HOST, port=DEFAULT_PORT, dbname=DEFAULT_DBNAME, user=DEFAULT_USER):
...
def _default_path():
    ...
    if platform.system() == "Windows":
        return path.join(
            os.getenv("APPDATA", path.join(path.expanduser("~"), "AppData")),
            "postgresql",
            "pgpass.conf",
        )
