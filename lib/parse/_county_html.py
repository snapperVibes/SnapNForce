__all__ = ["general_content", "tax_content"]
# fmt: off
...
def general_content(html_: AnyStr) -> tuple[_ScrapedData, _ScrapedData]:
    soup = _soupify(html_)
    owners = _find_owner_names(soup)
    mailing = _find_owner_mailing(soup)
    return owners, mailing

def tax_content(html_: AnyStr) -> tuple[_ScrapedData, _ScrapedData]:
    soup = _soupify(html_)
