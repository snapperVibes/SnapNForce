...
# """
# # TODO: YO, ERIC.
# #   The tables humanparcel and parcelmailingaddress need new unique indexes.
# #  humanparcel is especially hard because you can have multiple
# import itertools
# import logging
# import os.path
#
...
#     for link_ids, parcel_key, mailing_address_ids, linked_object_role_id in select_duplicate_parcel_mailing_addresses(conn, municode):
#         current = []
#         dups = []
#         for address_id in mailing_address_ids:
#             is_current = conn.execute(text("SELECT deactivatedts FROM mailingaddress WHERE addressid=:address_id").params({"address_id": address_id})).one()
#             if not is_current:
#                 dups.append(link)
#             current.append(address_id)
#
#         if len(current) != 1:
#             breakpoint()
#
...
#         """
#
#             SELECT array_agg(linkid) linkids, parcel_parcelkey, array_agg(mailingaddress_addressid), linkedobjectrole_lorid FROM parcelmailingaddress pma
#             GROUP BY parcel_parcelkey, linkedobjectrole_lorid, deactivatedts
#             HAVING count(*) > 1 AND deactivatedts IS null
#         """
#     )
#     if municode:
